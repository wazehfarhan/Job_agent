...
**Version:** 0.2 · **Status:** Draft · **Last reviewed:** 2026-09-21
...
| US-4 | As a user, I trigger (and later schedule) discovery so new postings land in my job DB. | P0 | ✅ manual (schedule = T3.1) |
| US-5 | As a user, each posting is parsed into structured fields (skills, salary, work mode, deadlines) I can trust. | P1 | ✅ |
| US-6 | As a user, I see a ranked shortlist (strong / possible / weak / ineligible) with reasons. | P1 | ✅ via API (UI = T2.5/T2.6) |
...
| US-10 | As a user, I see every application's status and full history timeline. | P1 | ✅ via API (UI = T2.6) |
...
- **FR-5** Preferences capture target roles, job/internship types, locations, work modes, min salary/stipend, industries, keywords, exclusions, min match score — the contract the Matching Agent consumes. ✅ (stored and consumed by matching)
...
- **FR-14** Scheduled discovery (Celery beat, ~daily). 🔲 (Celery app exists; T3.1)
- **FR-15** Per-user criteria (keywords/types) applied to discovery. 🟡 (criteria dataclass + keywords param; not user-wired)
...
- **FR-18** Content hash to detect posting changes. ✅ (computed at discovery + extraction)

### 8.6 Matching
- **FR-19** Rule-based eligibility first (location/work-mode/salary/type/exclusions) → hard filter, category `ineligible`. ✅
- **FR-20** Structured skill comparison (required vs owned). ✅
- **FR-21** Semantic similarity via pgvector embeddings of CV vs posting. ✅ (optional at runtime; heuristic fallback)
...
- **FR-23** Score, category, and reasons persisted on the application. ✅ (`match_score`, `match_category`, `match_reasons[]`)
...
- **FR-25** Application enters `awaiting_approval`; nothing is sent without explicit approval (`approved` → `submitting`). 🔲 (transition endpoint exists; the gate ships with the prep flow, v0.5)
...
### 8.9 Tracking
- **FR-28** `ApplicationStatus` lifecycle: discovered → analyzing → matched → preparing → awaiting_approval → approved → submitting → applied → interview / rejected / withdrawn / failed / needs_user_action. ✅ (enum + transition API; future stages drive the remaining states)
- **FR-29** Every transition appends a `status_events` row (append-only; history is never mutated). ✅
...
| NFR-3 | **Security:** bcrypt + JWT; uploaded files stored under random UUID names (path-traversal safe); size/type validated; secrets only via env; CORS via env. |
...
| NFR-7 | **Reliability:** request-scoped DB sessions; dedup idempotency; a failed source pull never corrupts the discovery run. |
| NFR-8 | **Testability:** each pipeline stage is a separate, unit-testable component (tests = T3.4). |
...
| Hand-written migration drift | Verify `alembic upgrade head` against live DB; future changes via autogenerate |
| Single-user scope creep | Keep per-user FKs; defer multi-user features explicitly |

## 13. Release plan

Phased — see [FutureUpdate.md](FutureUpdate.md). Current position: **v0.2–v0.4
shipped** (infrastructure, extraction, matching); next is v0.5 (preparation +
approval), then the frontend (v0.6).